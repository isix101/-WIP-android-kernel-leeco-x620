...
struct pinctrl_state *cam_ldo_vcamb_l = NULL;
struct pinctrl_state *cam_ldo_vcamb_h = NULL;
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	/* VCAMA - GPIO58 (Main Camera Analog Power) */
	cam_ldo_vcama_l = pinctrl_lookup_state(camctrl, "cam_ldo_vcama_0");
	cam_ldo_vcama_h = pinctrl_lookup_state(camctrl, "cam_ldo_vcama_1");

	/* VCAMB - GPIO110 (Sub Camera Analog Power) */
	cam_ldo_vcamb_l = pinctrl_lookup_state(camctrl, "cam_ldo_vcamb_0");
	cam_ldo_vcamb_h = pinctrl_lookup_state(camctrl, "cam_ldo_vcamb_1");
...
{
	if (IS_ERR_OR_NULL(camctrl)) {
		PK_ERR("%s: camctrl invalid\n", __func__);
		return -1;
	}

	switch (PwrType) {
	case RST:  /* CAM1 RST - GPIO35 */
		if (PinIdx == 1) {
			if (Val == 0)
	...
	case PDN:  /* CAM0 PDN - GPIO28 */
		if (PinIdx == 0) {
			if (Val == 0)
				pinctrl_select_state(camctrl, cam0_pnd_l);
			else
				pinctrl_select_state(camctrl, cam0_pnd_h);
			PK_DBG("CAM0 PDN: %s\n", Val ? "HIGH (sleep)" : "LOW (active)");
		}
		break;

	case AVDD:  /* GPIO58 (Main) / GPIO110 (Sub) */
		if (PinIdx == 0) {
			if (Val == 0)
			...
			if (Val == 0)
				pinctrl_select_state(camctrl, cam_ldo_vcamb_l);
			else
				pinctrl_select_state(camctrl, cam_ldo_vcamb_h);
			PK_DBG("CAM1 AVDD: %s\n", Val ? "ON" : "OFF");
		}
		break;
