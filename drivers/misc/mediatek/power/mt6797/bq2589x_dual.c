...
#define POWER_SUPPLY_PROP_DISABLE_QUICK_CHARGE 100
struct bq2589x
{
	struct device *dev;
	struct i2c_client *client;
	
	struct extcon_dev *extcon_otg;
...
{
    if (g_bq1 && g_bq1->extcon_otg)
        return extcon_get_cable_state_(g_bq1->extcon_otg, EXTCON_USB_HOST);
    return 0;
}

//extern int tusb320_check_type(void);
...
//extern int mtk_thermal_get_temp(void);
...
static void bq2589x_monitor_workfunc(struct work_struct *work)
{
	...
	if (bq->vbus_type == BQ2589X_VBUS_OTG)
	{
		if (g_bq1->extcon_otg && extcon_get_cable_state_(g_bq1->extcon_otg, EXTCON_USB_HOST) == 0)
		{
...
static int bq2589x_extcon_notify(struct notifier_block *nb, unsigned long event, void *ptr)
{
    struct bq2589x *bq = container_of(nb, struct bq2589x, extcon_nb);
    int id_status;

    if (event == EXTCON_USB_HOST) {
        id_status = extcon_get_cable_state_(bq->extcon_otg, EXTCON_USB_HOST);
        // Здесь пишем логику, аналогичную старой mtk_get_iddig_in_staus()
        if (id_status) {
            // OTG режим (ID пин замкнут на землю)
            // bq2589x_otg_mode(true);
        } else {
            // Обычный режим
            // bq2589x_otg_mode(false);
        }
    }
    return NOTIFY_OK;
}
								  ...
								  const struct i2c_device_id *id)
{
	...
	g_bq1->extcon_otg = extcon_get_extcon_dev("usb-id");
	if (!IS_ERR(g_bq1->extcon_otg)) {
		g_bq1->extcon_nb.notifier_call = bq2589x_extcon_notify;
		ret = extcon_register_notifier(g_bq1->extcon_otg, &g_bq1->extcon_nb);
		if (ret < 0)
			dev_err(&client->dev, "extcon register failed: %d\n", ret);
	} else {
		dev_info(&client->dev, "extcon usb-id not found\n");
	}
	...
	gpio_request(104, "bq2589x_irq");
	gpio_direction_input(104);
	irqn = gpio_to_irq(104);
...
//	gpio_request(56, "bq2589x_charger2_enable");
//	gpio_direction_output(56, 0);   hui
