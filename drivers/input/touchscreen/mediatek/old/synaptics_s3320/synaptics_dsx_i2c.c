                 ...
                 unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if ((full + 2) > msg_length) {
		kfree(msg);
		msg_length = full + 2;
		msg = kcalloc(msg_length, sizeof(struct i2c_msg), GFP_KERNEL);
	}
	...
	for (ii = 1; ii <= total; ii++) {
		msg[ii].addr = rmi4_data->i2c_client->addr;
		msg[ii].flags = I2C_M_RD;
		msg[ii].len = (ii == total) ? last : I2C_DMA_LIMIT;
		msg[ii].buf = (u8 *)(uintptr_t)gprDMABuf_pa + I2C_DMA_LIMIT * (ii - 1);
		msg[ii].ext_flag = (rmi4_data->i2c_client->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG);
		msg[ii].timing = 400;
	}
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		if (i2c_transfer(rmi4_data->i2c_client->adapter, msg, (total + 1)) == (total + 1)) {
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if(strstr(saved_command_line, "378C81")) //txd 
	{
