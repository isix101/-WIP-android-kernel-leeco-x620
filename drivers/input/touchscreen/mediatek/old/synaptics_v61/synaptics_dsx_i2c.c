...
#ifdef CONFIG_OF_TOUCH
unsigned int syna_touch_irq = 68;
u8 tpd_intr_type = 0;
unsigned int tpd_intr_pin = 0;
#endif

unsigned int tpd_rst_gpio_number = 57;
unsigned int tpd_int_gpio_number = 68;
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		//syna_touch_irq = irq_of_parse_and_map(node, 0);

		ret = request_irq(syna_touch_irq, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_LOW,
 ...
 */
 ...
 static irqreturn_t tpd_eint_handler(unsigned irq, struct irq_desc *desc)
 {
	disable_irq_nosync(syna_touch_irq);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if( synaptics_rmi4_i2c_read(rmi4_data,0x004,(unsigned char *)&status,sizeof(status)) !=1)
		rmi4_data->i2c_client->addr = 0x4b; //modify dvt-1 i2c addr 0x20 to 0x4b
...
static const struct of_device_id synaptics_i2c_dt_match[] = {
	//{.compatible = "mediatek,cap_touch"},
	//{.compatible = "mediatek,cap_touch_sy"},
	{.compatible = "synaptics,cap_touch"},
	{},
};

MODULE_DEVICE_TABLE(of, synaptics_i2c_dt_match);
...
#define TPD_DEVICE_SYNAPTICS    "synaptics-tpd"
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(TPD_DEVICE_SYNAPTICS, (TPD_I2C_ADDR))};
...
static int __init synaptics_rmi4_init(void)
{
	...
	printk("cxw synaptics_rmi4_init\n");
