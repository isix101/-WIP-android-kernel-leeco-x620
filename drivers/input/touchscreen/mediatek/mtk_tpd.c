...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	eint_as_int = pinctrl_lookup_state(pinctrl1, "state_eint_as_int0");
...
static int tpd_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    ...
    blank = *(int *)evdata->data;
    TPD_DMESG("EARLY fb_notify(blank=%d)\n", blank);
    
    switch (blank) {
    case FB_BLANK_POWERDOWN:
        TPD_DMESG("LCD OFF Notify (EARLY) - suspending touch BEFORE power off\n");
        if (g_tpd_drv) {
            err = cancel_work_sync(&touch_resume_work);
            if (!err)
                TPD_DMESG("cancel touch_resume_workqueue err = %d\n", err);
            g_tpd_drv->suspend(NULL);  // ← теперь вызывается ДО отключения питания
        }
        tpd_suspend_flag = 1;
        break;
    default:
