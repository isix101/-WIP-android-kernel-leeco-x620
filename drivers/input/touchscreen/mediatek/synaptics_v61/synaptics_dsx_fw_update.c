...
#define FW_IMAGE_TRULY "synaptics/s332U_LeTV_S2_Truly.img"
#define FW_IMAGE_BOE "synaptics/s332U_LeTV_S2_BOE.img"
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		/* Select firmware based on product_id */
		if (!strcmp(fwu->product_id_string, "S2-05-01")) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					FW_IMAGE_BOE, sizeof(FW_IMAGE_BOE),
					sizeof(FW_IMAGE_BOE));
		} else if (!strcmp(fwu->product_id_string, "S2-03-01")) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					FW_IMAGE_TRULY, sizeof(FW_IMAGE_TRULY),
					sizeof(FW_IMAGE_TRULY));
		} else {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: Invalid product_id : %s, leave FW update procedure\n",
					__func__, fwu->product_id_string);
			retval = -EINVAL;
			goto exit;
		}
