...
#define TPD_LOG(fmt, args...) printk("[SYNA_DEBUG] %s: " fmt, __func__, ##args)
...
static int tpd_irq_registration(void)
{
	...
	printk("[SYNA] %s: looking for touch_of_match node\n", __func__);
	node = of_find_matching_node(node, touch_of_match);
	if (node) {
		printk("[SYNA] %s: found node %s\n", __func__, node->full_name);

		of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
		printk("[SYNA] %s: debounce gpio=%d us=%d\n", __func__, ints[0], ints[1]);

		gpio_set_debounce(ints[0], ints[1]);

		touch_irq_dsx = irq_of_parse_and_map(node, 0);
		printk("[SYNA] %s: irq_of_parse_and_map returned %d\n", __func__, touch_irq_dsx);
		...
		printk("[SYNA] %s: request_irq ret=%d\n", __func__, ret);
	} else {
                        ...
                        unsigned short addr)
{
    ...
    if (page != rmi4_data->current_page) {
        ...
        retval = i2c_master_send(i2c, buf, PAGE_SELECT_LEN);
        if (retval != PAGE_SELECT_LEN) {
            dev_err(&i2c->dev, "%s: I2C set_page failed\n", __func__);
            return -EIO;
        }
        rmi4_data->current_page = page;
    }
                      ...
                      unsigned short addr, unsigned char *data, unsigned short length)
{
    ...
    if (retval != PAGE_SELECT_LEN) {
        dev_err(&client->dev, "%s: set_page failed for addr 0x%04x\n", __func__, addr);
        retval = -EIO;
        goto exit;
    }
    ...
    if (retval != 1) {
        dev_err(&client->dev, "%s: i2c_master_send failed, ret=%d\n", __func__, retval);
        retval = -EIO;
        goto exit;
    }
    
    while (left_len > 0) {
        chunk_size = (left_len > 8) ? 8 : left_len;
        
        retval = i2c_master_recv(client, pData, chunk_size);
        if (retval != chunk_size) {
            dev_err(&client->dev, "%s: i2c_master_recv failed, requested=%d, got=%d\n",
                    __func__, chunk_size, retval);
            retval = -EIO;
            goto exit;
        }
                       ...
                       unsigned short addr, unsigned char *data, unsigned short length)
{
    ...
    retval = tpd_set_page(rmi4_data, addr);
    if (retval != PAGE_SELECT_LEN) {
        dev_err(&client->dev, "%s: set_page failed\n", __func__);
        retval = -EIO;
        goto exit;
    }
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    if (!rmi4_data->i2c_enabled) {
        dev_dbg(&rmi4_data->i2c_client->dev,
                "%s: I2C disabled, skipping sensor report\n", __func__);
        return;
    }
    ...
    if (status.unconfigured && !status.flash_prog) {
        ...
        // ========== ЗАЩИТА: Проверяем I2C перед reinit ==========
        if (rmi4_data->i2c_enabled) {
    ...
    if (!list_empty(&exp_data.list)) {
        list_for_each_entry(exp_fhandler, &exp_data.list, link) {
            if (!exp_fhandler->insert && !exp_fhandler->remove &&
...
static int touch_event_handler(void *data)
{
    ...
    do {
        ...
        if (!rmi4_data->touch_stopped && rmi4_data->i2c_enabled) {
            synaptics_rmi4_sensor_report(rmi4_data);
        } else {
            dev_dbg(&rmi4_data->i2c_client->dev,
                    "%s: I2C disabled or touch stopped, skipping\n", __func__);
        }
...
static int synaptics_rmi4_irq_enable(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
    ...
    TPD_LOG("enable=%d, current irq_enabled=%d\n", enable, rmi4_data->irq_enabled);
    ...
    if (enable) {
        if (rmi4_data->i2c_enabled) {
            ...
            if (retval < 0) {
                TPD_LOG("ERROR: failed to clear interrupts, ret=%d\n", retval);
                return retval;
            }
            TPD_LOG("cleared interrupt status\n");
        } else {
            TPD_LOG("I2C disabled, skipping interrupt status clear\n");
        }

        if (!rmi4_data->irq_enabled) {
            TPD_LOG("calling tpd_gpio_as_int(1)\n");
//          tpd_gpio_as_int(1);
            TPD_LOG("calling enable_irq(%d)\n", touch_irq_dsx);
            enable_irq(touch_irq_dsx);

            rmi4_data->irq_enabled = true;
            TPD_LOG("irq_enabled set to true\n");
        }
    } else {
        if (rmi4_data->irq_enabled) {
            TPD_LOG("calling disable_irq_nosync(%d)\n", touch_irq_dsx);
            disable_irq_nosync(touch_irq_dsx);

            rmi4_data->irq_enabled = false;
            TPD_LOG("irq_enabled set to false\n");
        }
    }
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	// Мьютекс уже захвачен в f1a_init
...
static void synaptics_rmi4_exp_fn_work(struct work_struct *work)
{
    ...
    if (!rmi4_data->i2c_enabled) {
        dev_dbg(&rmi4_data->i2c_client->dev,
                "%s: I2C disabled, skipping expansion work\n", __func__);
        return;
    }
...
static void synaptics_rmi4_suspend(struct device *dev)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = g_rmi4_ptr;
	printk("Synaptics suspend start!");

	rmi4_data->i2c_enabled = false;
...
unsigned short force[] = {TPD_I2C_ADDR};
...
static int tpd_local_init(void)
{
	...
	if(i2c_add_driver(&tpd_i2c_driver) != 0) {
		TPD_LOG("ERROR: i2c_add_driver failed\n");
		return -ENODEV;
	}
	TPD_LOG("i2c_add_driver succeeded\n");

#ifdef TPD_HAVE_BUTTON
	tpd_button_setting(TPD_KEY_COUNT, tpd_keys_local, tpd_keys_dim_local);
	TPD_LOG("buttons configured\n");
#endif
...
static int __init synaptics_rmi4_init(void)
{
	...
	TPD_LOG("i2c_register_board_info done, bus=%d, addr=0x%x\n", TPD_I2C_BUS, TPD_I2C_ADDR);

	if(tpd_driver_add(&synaptics_rmi4_driver) < 0) {
		TPD_LOG("ERROR: tpd_driver_add failed\n");
		return -ENODEV;
	}
	TPD_LOG("tpd_driver_add succeeded\n");
